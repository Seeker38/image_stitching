...
class Stitcher:
    ...
    def stitch(self, imgs, detectKeypointAlgorithm = 'our', blending_mode = "linearBlending", ratio = 0.75):
        '''
            The main method to stitch image
        '''
        self.detectKeypointAlgorithm = detectKeypointAlgorithm
    ...
    def stitch_multi_image(self, imgs, detectKeypointAlgorithm = 'our', blending_mode = "linearBlending", ratio = 0.75):
        '''
            The main method to stitch image
        '''
        self.detectKeypointAlgorithm = detectKeypointAlgorithm
        ...
        queue = deque(imgs)
        stitched_image = queue.popleft()
        max_attempts = len(queue)

        while queue and max_attempts > 0:
            next_image = queue.popleft()
        
            kps_l, features_l = self.detectAndDescribe(stitched_image)
            kps_r, features_r = self.detectAndDescribe(next_image)

            matches_pos = self.matchKeyPoint(kps_l, kps_r, features_l, features_r, ratio)

            if len(matches_pos) < 4:
                queue.append(next_image)
                max_attempts -= 1
                continue

            HomoMat = self.fitHomoMat(matches_pos)
            stitched_image = self.warp([stitched_image, next_image], HomoMat, blending_mode).astype('uint8')
            max_attempts = len(queue)

        if queue:
            print("Không thể ghép tất cả các ảnh.")
    ...
    def detectAndDescribe(self, img):
        ...
        if self.detectKeypointAlgorithm == 'our':
            img = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
            kps, features = pysift.computeKeypointsAndDescriptors(img)
        elif self.detectKeypointAlgorithm == 'SIFT':
            sift = cv2.SIFT_create()
            kps, features = sift.detectAndCompute(img,None)
        elif self.detectKeypointAlgorithm == 'ORB':
            orb = cv2.ORB_create()
            kps, features = orb.detectAndCompute(img, None)

        elif self.detectKeypointAlgorithm == 'FAST':
            fast = cv2.FastFeatureDetector_create()
            kps = fast.detect(img, None)

        elif self.detectKeypointAlgorithm == 'KAZE':
            kaze = cv2.KAZE_create()
            kps, features = kaze.detectAndCompute(img, None)

        elif self.detectKeypointAlgorithm == 'AKAZE':
            akaze = cv2.AKAZE_create()
            kps, features = akaze.detectAndCompute(img, None)
        else:
            img = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
            kps, features = pysift.computeKeypointsAndDescriptors(img)
